        ...
        <div className="flex-1 w-full bg-transparent space-y-3">
          <div className="bg-white shadow-[0_1px_2px_0_rgba(0,0,0,0.2)] rounded-sm">
            ...
            <div className="divide-y divide-gray-100">
              {cartItems.map((item) => (
                <div key={item.key} className="p-4 sm:p-6 flex flex-col sm:flex-row gap-6">
                  {/* Left: Image & Qty */}
                        ...
                        {item.originalPrice && item.originalPrice > item.price && (
                          <>
                            <p className="text-xs text-gray-400 line-through">
                              {item.originalPrice.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
                            </p>
            ...
            <div className="p-4 border-t border-gray-100 flex justify-end bg-white shadow-[0_-2px_10px_0_rgba(0,0,0,0.05)] sticky bottom-0 z-10">
              <button
                ...
                className="bg-[#fb641b] hover:bg-[#fb641b]/90 text-white font-semibold text-base py-3 px-12 sm:px-16 uppercase tracking-wide rounded-sm shadow-sm transition-colors"
              >
                Place Order
              </button>
            </div>
          ...
          <div className="p-4 space-y-4 border-b border-gray-100">
            ...
            <div className="flex justify-between items-center text-sm text-black">
              ...
              <span className={delivery === 0 ? 'text-green-600' : ''}>
                {delivery === 0 ? 'FREE' : delivery.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}
              </span>
            </div>
          </div>
          <div className="p-4">
            <div className="flex justify-between items-center font-bold text-base text-black mb-3">
