...
export default function Cart() {
  ...
  if (cartItems.length === 0) return (
    <div className="min-h-[70vh] flex items-center justify-center bg-gray-50 px-6">
      <div className="text-center bg-white border border-gray-100 p-16 md:p-24 max-w-xl mx-auto shadow-2xl relative overflow-hidden rounded-xl">
        <div className="absolute top-0 left-0 right-0 h-1 bg-blue-600 shadow-[0_0_15px_rgba(37,99,235,0.3)]" />
        <div className="w-20 h-20 bg-gray-50 border border-gray-100 flex items-center justify-center mx-auto mb-10 rounded-xl">
          <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" fill="none" stroke="#2563EB" strokeWidth="1.5" viewBox="0 0 24 24">
    ...
    <div className="bg-gray-50 min-h-screen text-gray-800">
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="mb-20 text-center">
          <span className="text-blue-600 font-bold text-[8px] uppercase tracking-[0.4em] mb-4 block">Selection Archive</span>
          <h1 className="font-serif text-4xl md:text-6xl font-bold text-gray-900 tracking-tight">Shopping Bag</h1>
          <div className="w-12 h-px bg-blue-600/30 mx-auto mt-8" />
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          <div className="lg:col-span-8">
            <div className="bg-white border border-gray-100 p-8 shadow-2xl space-y-10 rounded-xl">
              {cartItems.map((item) => (
                <div key={item.key} className="group flex flex-col sm:flex-row gap-10 pb-10 border-b border-gray-100 last:border-0 last:pb-0">
                  <Link to={`/product/${item._id}`} className="shrink-0 overflow-hidden w-28 sm:w-36 bg-gray-50 border border-gray-100 shadow-xl pulse-photo rounded-xl" style={{aspectRatio:'3/4'}}>
                    <img
                  ...
                  <div className="flex-1 flex flex-col justify-between py-2">
                    <div>
                      <div className="flex justify-between gap-6 mb-4">
                        <Link to={`/product/${item._id}`} className="font-serif text-2xl font-bold text-gray-900 hover:text-blue-600 transition-colors tracking-tight">{item.name}</Link>
                        <p className="font-serif font-bold text-gray-900 text-xl tracking-tight">
                      ...
                      <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-blue-600 mb-8">{item.subcategory || item.category}</p>
                      
                      <div className="flex flex-wrap gap-4 text-[9px] font-bold uppercase tracking-widest">
                        {item.size && (
                           <span className="bg-gray-50 px-4 py-2 border border-gray-100 text-gray-500 rounded-xl">Size: {item.size}</span>
                        )}
                        {item.color && (
                           <span className="bg-gray-50 px-4 py-2 border border-gray-100 text-gray-500 rounded-xl">Color: {item.color}</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-12">
                      <div className="flex items-center bg-gray-50 p-1 border border-gray-100 rounded-xl">
                        <button onClick={() => updateCartQty(item._id, item.size, item.color, item.quantity - 1)}
                          className="w-10 h-10 flex items-center justify-center text-gray-400 hover:text-blue-600 hover:bg-white transition-all text-xl" disabled={item.quantity <= 1}>−</button>
                        <span className="w-14 h-10 flex items-center justify-center text-sm font-bold text-gray-900">{item.quantity}</span>
                        <button onClick={() => updateCartQty(item._id, item.size, item.color, item.quantity + 1)}
                          className="w-10 h-10 flex items-center justify-center text-gray-400 hover:text-blue-600 hover:bg-white transition-all text-xl">+</button>
                      </div>
          ...
          <div className="lg:col-span-4">
            <div className="bg-white p-10 sticky top-32 border border-gray-100 shadow-2xl relative overflow-hidden group rounded-xl">
              ...
              <div className="relative z-10">
                ...
                {(() => {
                  ...
                  return (
                    <div className="mb-10 bg-blue-50 border border-blue-100 rounded-xl p-5">
                      {remaining > 0 ? (
                        <p className="text-[10px] font-bold uppercase tracking-widest text-blue-700 mb-3">
                          Add {remaining.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })} more for
                          <span className="text-blue-900"> Free Shipping</span>
                        </p>
                      ) : (
                ...
                <Link to="/collection" className="block text-center mt-10 text-[9px] font-bold uppercase tracking-[0.4em] text-gray-400 hover:text-blue-600 transition-colors">
                  Continue Browsing
                </Link>
              </div>
            </div>
